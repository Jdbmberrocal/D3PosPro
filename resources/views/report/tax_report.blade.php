                        ...
                        <table class="table table-bordered table-striped" id="input_tax_table">
                            ...
                            <tfoot>
                                <tr class="bg-gray font-17 text-center footer-total">
                                    ...
                                    @foreach($taxes as $tax)
                                    <td>
                                            <span class="display_currency" id="base_input_{{$tax['id']}}" data-currency_symbol="true"></span>
                                        </td>
                                        <td>
    ...
    $(document).ready(function() {
        ...
        input_tax_table = $('#input_tax_table').DataTable({
            ...
            columns: [
                ...
                @foreach($taxes as $tax)
                { data: "tax_{{$tax['id']}}_base", searchable: false, orderable: false },
                { data: "tax_{{$tax['id']}}", searchable: false, orderable: false }, //esta data aun no llega debo mandarlo aka para que se pueda totalisar
                @endforeach
                { data: "Excluido", searchable: false, orderable: false },
                @foreach($taxesWithZeroValue as $tax)
                { data: "tax_{{$tax['id']}}_exento", searchable: false, orderable: false },
                @endforeach
                ...
                if (typeof (output_tax_datatable) == 'undefined') {
                    output_tax_datatable = $('#output_tax_table').DataTable({
                        ...
                        columns: [
                            ...
                            @foreach($taxes as $tax)
                            { data: "tax_{{$tax['id']}}_base", searchable: false, orderable: false },
                            { data: "tax_{{$tax['id']}}", searchable: false, orderable: false },///este dato que llega de ajax llegaria a ser la base de este impuesto
                            @endforeach
                            { data: "Excluido", searchable: false, orderable: false },
                            @foreach($taxesWithZeroValue as $tax)
                            { data: "tax_{{$tax['id']}}_exento", searchable: false, orderable: false },
                        ...
                        fnDrawCallback: function(oSettings) {
                            ...
                            @foreach($taxes as $tax)
                                ...
                                $("#base_input_{{$tax['id']}}").text(
                                    sum_table_col($('#input_tax_table'), "tax_{{$tax['id']}}_base")
                                );
                            @endforeach
