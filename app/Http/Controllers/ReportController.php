...
class ReportController extends Controller
{
     ...
     * @return \Illuminate\Http\Response
     */
    public function getTaxDetailsNew(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $sells = Transaction::leftJoin('tax_rates as tr', 'transactions.tax_id', '=', 'tr.id')
                ...
                ->where(function ($query) {
                    $query->whereNull('transactions.tax_id')
                        ->orWhere('transactions.tax_id', '=', 0);
                })
                ->with(['payment_lines'])
            ...
            if ($type == 'sell') {
                $sells->where('transactions.type', 'sell')
                    ...
                    ->with(['sell_lines' => function ($q) {
                        // $q->whereNotNull('transaction_sell_lines.tax_id');
                        $q->orWhereNull('transaction_sell_lines.tax_id'); // esta correccion traera los  registros cuyo valor en impuestos sea null
                    }, 'sell_lines.line_tax']);
            ...
            if ($type == 'purchase') {
                $sells->where('transactions.type', 'purchase')
                    ...
                    // ->where(function ($query) { //funcion original
                    //     $query->whereHas('purchase_lines', function ($q) {
                    //         $q->whereNotNull('purchase_lines.tax_id');
                    //     })->orWhereNotNull('transactions.tax_id');
                    // })
                    ->with(['purchase_lines' => function ($q) {
                        // $q->whereNotNull('purchase_lines.tax_id');//original
                        $q->orWhereNull('purchase_lines.tax_id'); /// esto actuliza  para obtener incluso los valores null en impuestos en la linea de compreas 
            ...
            //logica para evaluar  los impuestos que  sean de valor 0 es decir que sean exentos
            $taxesTotal = TaxRate::forBusiness($business_id);
            $taxesWithZeroValue = [];
            $taxes = [];
            // Separar los elementos en los dos arrays
            foreach ($taxesTotal as $tax) {
                if ($tax['amount'] === 0.0) {
                    $taxesWithZeroValue[] = $tax;
                } else {
                    $taxes[] = $tax;
                }
            }
        //    dd($taxesWithZeroValue);
            foreach ($taxes as $tax) {
                ...
                $datatable->addColumn($col, function ($row) use ($tax, $type, $col, $group_taxes) {
                    ...
                    if ($type == 'sell') {
                        ...
                        // dd("imprimir valores de otra instancai base ", count($row->purchase_lines) , $row->purchase_lines  );
                    ...
                    // dd($row->tax_id ,$tax['id']);
                    if ($tax_amount > 0 ) {
                        return '<span class="display_currency '.$col.'" data-currency_symbol="true" data-orig-value="'.$tax_amount.'">'.$tax_amount.'</span>';
                    } else {
                        return '';
                        //cambio agregado para mostrar en 0  cuando no se aplique ningun tipo de impuesto
                        // return '<span class="display_currency '.$col.'" data-currency_symbol="true" data-orig-value="0">0</span>';
                    }

                });


                $datatable->addColumn($col.'_base', function ($row) use ($tax, $type, $col, $group_taxes) {
                    $tax_amount = 0;
                    $tax_base = 0;
                    if ($type == 'sell') {
                        foreach ($row->sell_lines as $sell_line) {
                            if ($sell_line->tax_id == $tax['id']) {
                                $tax_amount += ($sell_line->item_tax * ($sell_line->quantity - $sell_line->quantity_returned));
                                $tax_base += ($sell_line->quantity - $sell_line->quantity_returned) * $sell_line->unit_price;
                            }
                        ...
                        foreach ($row->purchase_lines as $purchase_line) {
                            ...
                            if ($purchase_line->tax_id == $tax['id']) {
                                $tax_amount += ($purchase_line->item_tax * ($purchase_line->quantity - $purchase_line->quantity_returned));
                                $tax_base += ($purchase_line->quantity - $purchase_line->quantity_returned) * $purchase_line->purchase_price;
                                // dd($purchase_line->unit_price);
                            }
                    ...
                    if ($row->tax_id == $tax['id']) {
                        $tax_amount += $row->tax_amount;
                        $tax_base += $row->total_before_tax;
                    }
                    // dd($row->tax_id, $tax['id'], $group_taxes[$row->tax_id]['sub_taxes'], $group_tax_details);

                    // dd($tax_amount, $tax_base);
                    //break group tax
                    if (! empty($group_taxes[$row->tax_id]) && array_key_exists($tax['id'], $group_taxes[$row->tax_id]['sub_taxes'])) {
                        $group_tax_details = $this->transactionUtil->groupTaxDetails($row->tax_id, $row->tax_amount);
                        ...
                        foreach ($group_tax_details as $sub_tax_details) {
                            if ($sub_tax_details['id'] == $tax['id']) {
                                $sub_tax_share = $sub_tax_details['calculated_tax'];
                                $sub_tax_base = $sub_tax_details['calculated_base'];
                            }
                        }

                        $tax_amount += $sub_tax_share;
                        $tax_base += $sub_tax_base; 
                    }
                  
                 
                    if ($tax_base > 0) {
                        return '<span class="display_currency '.$col.'_base'.'" data-currency_symbol="true" data-orig-value="'.$tax_base.'">'.$tax_base.'</span>';
                    } else {
                        return '';
                        //cambio agregado para mostrar en 0  cuando no se aplique ningun tipo de impuesto
                        // return '<span class="display_currency '.$col.'" data-currency_symbol="true" data-orig-value="0">0</span>';
            ...
            /////iterar  para los valores que sean exentos 
            foreach ($taxesWithZeroValue as $tax) {
                $col = 'tax_'.$tax['id'];
                // $raw_cols[] = $col;
                $raw_cols[] = $col.'_exento';
               
                $datatable->addColumn($col.'_exento', function ($row) use ($tax, $type, $col, $group_taxes) {
                    $tax_amount = 0;
                    $tax_base = 0;
                    if ($type == 'sell') {
                        foreach ($row->sell_lines as $sell_line) {
                    ...
                    if ($tax_base > 0 ) {
                        // Si $tax_base es mayor que 0 o si el impuesto es exento, mostrar el valor base
                        return '<span class="display_currency '.$col.'_exento'.'" data-currency_symbol="true" data-orig-value="'.$tax_base.'">'.$tax_base.'</span>';
                    } else {
                        // Si $tax_base es 0 y el impuesto tiene un valor mayor que 0, mostrar el valor del impuesto
                        return '';
                    }
                });
            }
            ///////fin iterar para los valores que sean excentos
            
            ////////iterar los valores que son excluidos es decir aquellos que no tienen impuesto agregados 
           
            // dd($datatable);
            // foreach ($taxes as $tax) {
                $col = 'Excluido';
                $raw_cols[] = $col;
                $raw_cols[] = $col.'_excluido';
               
                $datatable->addColumn($col , function ($row) use ($type, $col, $group_taxes) {
                    $tax_amount = 0;
                    $tax_base = 0;
                    if ($type == 'sell') {
                        foreach ($row->sell_lines as $sell_line) {
                                // dd($sell_line);
                            if ($sell_line->tax_id == null) {
                                // dd($sell_line->tax_id);
                                $tax_amount += ($sell_line->quantity - $sell_line->quantity_returned) * $sell_line->unit_price;
                                $tax_base += ($sell_line->quantity - $sell_line->quantity_returned) * $sell_line->unit_price;
                            }
                        ...
                        // dd(count($row->purchase_lines), $row);
                        foreach ($row->purchase_lines as $purchase_line) {
                            // dd($purchase_line);
                            if ($purchase_line->tax_id == null) {
                                // dd("hay un id de valor null");

                                
                                $tax_amount += ($purchase_line->quantity - $purchase_line->quantity_returned) * $purchase_line->purchase_price;
                                // $tax_amount += (($purchase_line->purchase_price_inc_tax - $purchase_line->quantity_returned));
                                $tax_base += ($purchase_line->quantity - $purchase_line->quantity_returned) * $purchase_line->purchase_price;
                                // dd($purchase_line->unit_price);
                            }
            ...
            //////////fin iterarar los valores que son excluidos 
     ...
     * @return \Illuminate\Http\Response
     */
    public function getTaxReportview(Request $request)
    {
        if (! auth()->user()->can('tax_report.view')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    //en el codigo origianl se mandaba solo los impuestos pero ahora para tratarlos estoy mandando los impuestos  que no son 0 separados y los que  son diferentes mandarlos aparte en otro array
            $taxesTotal = TaxRate::forBusiness($business_id);
            $taxesWithZeroValue = [];
            $taxes = [];
            // Separar los elementos en los dos arrays
        ...
        //dd($taxes);
        return view('report.tax_report')
            ->with(compact('business_locations', 'taxes', 'tax_report_tabs', 'taxesWithZeroValue','contact_dropdown'));
    }
